...
Class Mazes
  ...
  Public Overrides Function OnUserUpdate(elapsedTime As Single) As Boolean
        ...
        For py = 0 To m_pathWidth - 1
          For px = 0 To m_pathWidth - 1
            If (m_maze(y * m_mazeWidth + x) And CellPath.Visited) <> 0 Then
              Draw(x * (m_pathWidth + 1) + px, y * (m_pathWidth + 1) + py, Solid, FgWhite) ' Draw Cell
            Else
              Draw(x * (m_pathWidth + 1) + px, y * (m_pathWidth + 1) + py, Solid, FgBlue) ' Draw Cell
            End If
          Next
        Next
    ...
    For py = 0 To m_pathWidth - 1
      For px = 0 To m_pathWidth - 1
        Draw(m_stack.Peek.X * (m_pathWidth + 1) + px, m_stack.Peek.Y * (m_pathWidth + 1) + py, &H2588, FgGreen) ' Draw Cell
      Next
    Next
